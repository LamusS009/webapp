...
with c2:
    ...
    st.subheader("***4. ¿Está reducida a manera escalonada correctamente esta matriz?***")
    st.latex(r"\begin{bmatrix} 2 & 3 & 4\\ 0 & 1 & 5\\ 0 & 0 & 3\end{bmatrix}")
    rs9 = st.selectbox(" ", ["Sí", "No, no es posible reducir esta matriz", "No"], key=9, index=None, placeholder="Seleccione una opción")
...
st.subheader("***1. ¿Cual es la ecuacion de la recta de la grafica (2)***")
x = np.linspace(-8,8,50)
y3 = (3+(2*x))/4
y4 = (3-(5*x))/(-3)
fig, ax = plt.subplots()
ax.plot(x,y3, label="Grafica 1")
ax.plot(x,y4, label="Grafica 2")
ax.grid()
ax.legend()
ax.axhline(0,color="black",linewidth=1)
ax.axvline(0,color="black",linewidth=1)
...
preg = [rs1,rs2,rs3,rs4,rs5,rs9, rs6,rs7,rs8]

cont = 0
if st.button("Revisar su puntuación"):
    for p, r in  zip(preg, respuesta.values()):
        if p == r:
            st.success(f"La respuesta es correcta ¡Felicidades!")
            cont +=1
        else:
            st.error(f"La respuesta es incorrecta. La respuesta es {r}")
            cont = cont
    if cont < 5:
        st.error(f"""Su puntacion fue {cont}/10 ¡puedes hacerlo mejor!""")
    else:
        st.balloons()
        st.success(f"""Su puntacion fue {cont}/10 ¡Felicidades!""")